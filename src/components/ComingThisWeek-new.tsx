...
export function ComingThisWeek() {
  ...
  const fetchComingThisWeek = async () => {
    try {
      ...
      // Request more items to ensure we get a good mix of both movies and TV shows
      ...
      if (finalMovies.length > 0 || finalSeries.length > 0) {
        ...
        if (recentData.results && recentData.results.length > 0) {
          ...
          const sampleMovies = sampleData.filter((item: any) => item.type === 'movie');
          const sampleSeries = sampleData.filter((item: any) => item.type === 'series');
      ...
      const errorMovies = sampleData.filter((item: any) => item.type === 'movie');
      const errorSeries = sampleData.filter((item: any) => item.type === 'series');
